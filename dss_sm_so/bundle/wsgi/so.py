...
class ServiceOrchestratorDecision(service_orchestrator.Decision, threading.Thread):
    ...
    def run(self):
        ...
        resp = self.configure.sendRequestToStatusUI(self.configure.ui_url + '/v1.0/service_ready', 'PUT', '{"user":"UI","token":"' + self.configure.ui_token + '","components":[{"name":"so"}]}')
        ...
        if self.so_e.templateManager.aaa_enable == "true":
            resp = self.configure.sendRequestToStatusUI(self.configure.ui_url + '/v1.0/service_ready', 'PUT', '{"user":"UI","token":"' + self.configure.ui_token + '","components":[{"name":"cms1"},{"name":"mcr"},{"name":"db"},{"name":"slaaas"},{"name":"aaaaas"},{"name":"lbaas"}]}')
        else:
            resp = self.configure.sendRequestToStatusUI(self.configure.ui_url + '/v1.0/service_ready', 'PUT', '{"user":"UI","token":"' + self.configure.ui_token + '","components":[{"name":"cms1"},{"name":"mcr"},{"name":"db"},{"name":"slaaas"},{"name":"lbaas"}]}')
        writeLogFile(self.swComponent,"Request response is:" + str(resp),'','')
        ...
        while(1):
            ...
            if scaleTriggered == True:
                ...
                if zHostToDelete is not None:
                    ...
                    resp = self.configure.sendRequestToStatusUI(self.configure.ui_url + '/v1.0/service_ready', 'DELETE', '{"user":"UI","token":"' + self.configure.ui_token + '","components":[{"name":"cms' + str(self.so_e.templateManager.numberOfCmsInstances+1) + '"}]}')
                    writeLogFile(self.swComponent,"Request response is:" + str(resp),'','')
                else:
                    resp = self.configure.sendRequestToStatusUI(self.configure.ui_url + '/v1.0/service_ready', 'PUT', '{"user":"UI","token":"' + self.configure.ui_token + '","components":[{"name":"cms' + str(self.so_e.templateManager.numberOfCmsInstances) + '"}]}')
                    writeLogFile(self.swComponent,"Request response is:" + str(resp),'','')
                ...
                if checkList[item] == "unknown":
                    ...
                    while (response_status < 200 or response_status >= 400):
                        ...
                        if "False" not in instanceInfo.values():
                            ...
                            resp = self.configure.sendRequestToStatusUI(self.configure.ui_url + '/v1.0/service_ready', 'POST', '{"user":"UI","token":"' + self.configure.ui_token + '","components":[{"name":"cms' + str(self.so_e.templateManager.numberOfCmsInstances) + '"}]}')
                            writeLogFile(self.swComponent,"Request response is:" + str(resp),'','')
    ...
    def run(self):
        ...
        while self.dependencyStat["DNS"] != "ready":
            if self.so_e.templateManager.dns_enable == 'true':
                ...
                try:
                    ...
                    resp = self.sendRequestToStatusUI(self.ui_url + '/v1.0/service_ready', 'PUT', '{"user":"UI","token":"' + self.ui_token + '","components":[{"name":"dnsaas"}]}')
                ...
                if self.so_e.dnsObject is not None:
                    ...
                    self.dependencyStat["DNS"] = "ready"
                    resp = self.sendRequestToStatusUI(self.ui_url + '/v1.0/service_ready', 'POST', '{"user":"UI","token":"' + self.ui_token + '","components":[{"name":"dnsaas"}]}')
                else:
        ...
        if self.so_e.templateManager.icn_enable == 'true':
            ...
            while self.dependencyStat["ICN"] != "ready":
                if self.so_e.icn_endpoint != None:
                    ...
                    resp = self.sendRequestToStatusUI(self.ui_url + '/v1.0/service_ready', 'PUT', '{"user":"UI","token":"' + self.ui_token + '","components":[{"name":"icnaas"}]}')
                    writeLogFile(self.swComponent,"Service_ready response is:" + str(resp),'','')
        ...
        while self.dependencyStat["MON"] != "ready":
            if self.so_e.monitoring_endpoint != None:
                ...
                resp = self.sendRequestToStatusUI(self.ui_url + '/v1.0/service_ready', 'PUT', '{"user":"UI","token":"' + self.ui_token + '","components":[{"name":"monaas"}]}')
                writeLogFile(self.swComponent,"Service_ready response is:" + str(resp),'','')
        ...
        if self.so_e.templateManager.aaa_enable == 'true':
            resp = self.sendRequestToStatusUI(self.ui_url + '/v1.0/service_ready', 'POST', '{"user":"UI","token":"' + self.ui_token + '","components":[{"name":"cms1"},{"name":"mcr"},{"name":"db"},{"name":"slaaas"},{"name":"aaaaas"},{"name":"lbaas"}]}')
            writeLogFile(self.swComponent,"Service_ready response is:" + str(resp),'','')
        else:
            resp = self.sendRequestToStatusUI(self.ui_url + '/v1.0/service_ready', 'POST', '{"user":"UI","token":"' + self.ui_token + '","components":[{"name":"cms1"},{"name":"mcr"},{"name":"db"},{"name":"slaaas"},{"name":"lbaas"}]}')
            writeLogFile(self.swComponent,"Service_ready response is:" + str(resp),'','')
        ...
        resp = self.sendRequestToStatusUI(self.ui_url + '/v1.0/service_ready', 'POST', '{"user":"UI","token":"' + self.ui_token + '","components":[{"name":"monaas"}]}')
        writeLogFile(self.swComponent,"Service_ready response is:" + str(resp),'','')
    ...
    def performDNSConfig(self):
        ...
        for item in self.instances:
            if item == "mcn.dss.lb.endpoint":
                ...
                #mcrDomainExists = self.so_e.dnsObject.get_domain(self.dssMcrDomainName, self.so_e.token)
                #writeLogFile(self.swComponent,mcrDomainExists.__repr__(), '', '')
                #if mcrDomainExists['code'] == 404:
                #    result = -1
                #    while (result != 1):
                #        time.sleep(2)
                #        result = self.so_e.dnsObject.create_domain(self.dssMcrDomainName, "info@dss-test.es", 3600, self.so_e.token)
                #        writeLogFile(self.swComponent,result.__repr__(), '', '')
                #        writeLogFile(self.swComponent,'DNS domain creation attempt for:' + str(self.instances[item]) , '', '')
                #    writeLogFile(self.swComponent,'DNS domain created for: ' + str(self.instances[item]) , '', '')
                #else:
                #    writeLogFile(self.swComponent,'DNS domain already exists for:' + str(self.instances[item]) , '', '')
                mcrRecordExists = self.so_e.dnsObject.get_record(domain_name=self.dssMcrDomainName, record_name=self.dssMcrRecordName, record_type='A', token=self.so_e.token)
                if mcrRecordExists.get('code', None) is not None and mcrRecordExists['code'] == 404:
                    result = -1
                    while (result != 1):
                        time.sleep(2)
                        result = self.so_e.dnsObject.create_record(domain_name=self.dssMcrDomainName, record_name=self.dssMcrRecordName, record_type='A', record_data=self.instances[item], token=self.so_e.token)
                        writeLogFile(self.swComponent,result.__repr__(), '', '')
                        writeLogFile(self.swComponent,'DNS record creation attempt for:' + str(self.instances[item]) , '', '')
                    writeLogFile(self.swComponent,'DNS record created for: ' + str(self.instances[item]) , '', '')
                else:
