...
def main(argv):
    try:
        ...
        print ("Usage: python icn_performance_tool.py -u <URL_TO_POLL_FROM> -e <DSS_CMS_IP> -i <ICN_API_URL> -c [Number of desired interests per sec: 300] -f [PATH_TO_SAVE-FILES default: ./] -p [ICN_PREFIX default: /dss] -q [dns_qps]")
        sys.exit(0)
    ...
    for opt, arg in opts:
        if opt == '-h':
            print ("Usage: python icn_performance_tool.py -u <URL_TO_POLL_FROM> -e <DSS_CMS_IP> -i <ICN_API_URL> -c [Number of desired interests per sec: 300] -f [PATH_TO_SAVE-FILES default: ./] -p [ICN_PREFIX default: /dss]")
            sys.exit(0)
        elif opt in ("-u", "--url"):
            url_to_poll = arg
