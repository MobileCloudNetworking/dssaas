...
class BroadcastManager:
    ...
    def __init__(self, file_manager, torrent_manager):
        ...
        self.log.debug('TorrentList:' + str(self.tm.getTorrentList()))
        self.log.debug('TrackerList:' + str(self.tm.getTrackerList()))

    def send_broadcast_message(self):
        s = socket(AF_INET, SOCK_DGRAM)
        s.setsockopt(SOL_SOCKET, SO_BROADCAST, 1)
        s.setsockopt(SOL_SOCKET, SO_REUSEADDR, 1)
        data = 'udp://' + str(self.tm.getSelfTracker()) +':6969/announce' + '!' + repr(time.time())
        for torrent in self.tm.getTorrentList():
            self.log.debug('Checking torrent: ' + str(torrent))
            data += '!' + str(torrent) + '!' + str(self.tm.getTorrentContent(torrent))
        data += '\n'
        self.log.debug(data)
    ...
    def sendAmazon_broadcast_message(self):
        ...
        s.setsockopt(SOL_SOCKET, SO_REUSEADDR, 1)
        data = 'udp://' + str(self.tm.getSelfTracker()) +':6969/announce' + '!' + repr(time.time())
        self.log.debug(str(self.tm.getTorrentList()))
        for torrent in self.tm.getTorrentList():
            self.log.debug('Checking torrent: ' + str(torrent))
            data += '!' + str(torrent) + '!' + str(self.tm.getTorrentContent(torrent))
        data += '\n'
    ...
    def parse_broadcast_message(self,message):
        ...
        if (msg_list[0] not in self.tm.getTrackerList()):
            self.tm.addTracker(msg_list[0])
            ...
            for filename in self.fm.list_files(self.path,['.webm'])[1]:
                self.tm.createTorrent(self.path,filename.split('.')[0]+'.torrent')
        for index in range (2, len(msg_list),2):
            ...
            if (torrent_name not in self.tm.getTorrentList()):
