...
class Application:
    ...
    def __init__(self):
        ...
        self.configurationstatus = {"provision":"False", "mon":"False", "dns":"False"}
    ...
    def __call__(self, environ, start_response):
        ...
        if environ['PATH_INFO'] == '/v1.0/test':
            return self.test()
        elif environ['PATH_INFO'] == '/v1.0/DB':
            return self.check_db_status()
    ...
    def check_db_status(self):
        if self.environ['REQUEST_METHOD'] == 'POST':
            ...
            jsonm = JSONManager()
            jsonm.jprint(body)
            db_json = jsonm.read(body)
            if (db_json == -1):
                return self.servererror(self.SERVER_ERROR_PARSE_JSON)
            if not (db_json["user"]==self.alloweduser and db_json["token"]==self.allowedtoken):
                return self.unauthorised()
            try:
